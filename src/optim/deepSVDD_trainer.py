...
class DeepSVDDTrainer(BaseTrainer):
  ...
  def __init__(self, objective, R, c, nu: float, optimizer_name: str = 'adam',
      lr: float = 0.001, n_epochs: int = 150,
      lr_milestones: tuple = (), batch_size: int = 128,
      weight_decay: float = 1e-6, device: str = 'cuda',
      n_jobs_dataloader: int = 0):
    super().__init__(optimizer_name, lr, n_epochs, lr_milestones, batch_size,
                     weight_decay, device,
                     n_jobs_dataloader)

    assert objective in ('one-class',
                         'soft-boundary'), "Objective must be either 'one-class' or 'soft-boundary'."
    self.objective = objective

    # Deep SVDD parameters
    self.R = torch.tensor(R,
                          device=self.device)  # radius R initialized with 0 by default.
  ...
  def train(self, dataset: BaseADDataset, net: BaseNet):
    ...
    for epoch in range(self.n_epochs):
      ...
      if epoch in self.lr_milestones:
        logger.info('  LR scheduler: new learning rate is %g' % float(
            scheduler.get_lr()[0]))
      ...
      for data in train_loader:
        ...
        if self.it % val_every == 0:
          self.validate(dataset, net, self.best_metric)
          net.train()
  ...
  def validate(self, dataset: BaseADDataset, net: BaseNet, best_metric):
    ...
    with torch.no_grad():
      for data_all in val_all_loader:
        ...
        accuracy = torch.mean((classifications == labels.float()).float())
